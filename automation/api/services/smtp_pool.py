...
async def send_via_pool(
    ...
    if not providers:
        # Check if there are ANY providers configured (vs all exhausted)
        async with async_session_factory() as db:
            total = await db.execute(select(SmtpProvider.id).limit(1))
            has_any = total.first() is not None
        if not has_any:
            # No providers configured — let caller fall back to legacy SMTP
            raise RuntimeError("No SMTP providers configured")

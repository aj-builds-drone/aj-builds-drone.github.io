"""
AJ Builds Drone — IMAP Bounce Checker.
...
logger = logging.getLogger("drone.bounce_checker")
...
async def check_bounces() -> dict:
    ...
    # Debug: log extracted addresses so we can see what IMAP found
    unique_addrs = sorted({h["addr"] for h in hits})
    unique_addrs_set = set(unique_addrs)
    logger.info("Bounce check: found %d NDRs with %d unique addresses", len(hits), len(unique_addrs))

    # Phase 2: Process results with async DB
    from api.services.drone_cadence_engine import handle_bounce

    # Cross-reference: check which of our sent prospect emails appear in bounces
    async with async_session_factory() as db:
        sent_rows = (await db.execute(
            select(DroneProspect.email)
            .join(OutreachEmail, OutreachEmail.prospect_id == DroneProspect.id)
            .where(OutreachEmail.status == "sent")
        )).all()
        our_sent = {r[0].lower() for r in sent_rows if r[0]}
    overlap = unique_addrs_set & our_sent
    if overlap:
        logger.info("Bounce check: matched %d of our sent addresses: %s", len(overlap), overlap)
    else:
        logger.info("Bounce check: none of our %d sent addresses appear in NDRs (NDRs are from other campaigns)", len(our_sent))
    ...
    for hit in hits:
        ...
        try:
                ...
                prospect_row = (
                    await db.execute(
                        select(DroneProspect.id).where(
                            DroneProspect.email == addr,
                        ).limit(1)
                    )
                ).first()
                if not prospect_row:
                    logger.debug("Bounce for %s — no matching prospect", addr)
                    result["not_found"] += 1
                    continue

                prospect_id = str(prospect_row[0])

                # Find the most recent sent email to this prospect
                email_row = (
                    await db.execute(
            ...
            p_name = prospect.name if prospect else "Unknown"
            result["bounced"] += 1
            ...
            logger.info("📬 Bounce detected: %s (%s) — marked bounced", addr, p_name)
...
async def check_replies() -> dict:
    ...
    for addr in replied_addrs:
        ...
        try:
            async with async_session_factory() as db:
                prospect = await db.get(DroneProspect, info["id"])
                if not prospect:
                    continue
                ...
                prospect.notes = (prospect.notes or "") + (
                    f"\n📩 Reply detected via IMAP — auto-set to replied (was {old_status})"
                )
            ...
            try:
                ...
                await _send_telegram_message(
                    f"📩 *Reply detected:* {_esc_md(info['name'])}\n"
                    f"📧 From: {_esc_md(addr)}\n"
                    f"🤝 Auto\\-set to replied — {cancelled} emails cancelled"
                )
            except Exception:
                pass

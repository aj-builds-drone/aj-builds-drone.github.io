...
async def record_open(tracking_id: str) -> bool:
    ...
    from api.models.prospect import OutreachEmail, DroneProspect
    ...
    async with async_session_factory() as db:
        ...
        if email.open_count == 1 and prospect:
            ...
            await _send_telegram_message(
                f"\U0001f440 *{_esc_md(prospect.name)}* opened your email\\!\n"
                f"\U0001f4cb Step {email.sequence_step}: {_esc_md(email.subject[:50])}\\.\\.\\."
            )
...
async def record_click(tracking_id: str, url: str) -> bool:
    ...
    async with async_session_factory() as db:
        ...
        if email.click_count == 1 and prospect:
            from api.services.notify import _send_telegram_message, _esc_md
            await _send_telegram_message(
                f"\U0001f517 *{_esc_md(prospect.name)}* clicked a link\\!\n"
                f"\U0001f4cb Step {email.sequence_step}\n"
                f"\U0001f310 URL: {_esc_md(url[:80])}"
            )
